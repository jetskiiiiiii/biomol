...
def rabbits_and_recurrence_relations(months: int, multiplier: int) -> int:
    ...
    if months <= 2:
        ...
        return rabbits_and_recurrence_relations(months - 1, multiplier) + (
            rabbits_and_recurrence_relations(months - 2, multiplier) * multiplier
        )
...
def mendels_first_law(
    ...
    total_offspring = (
        math.factorial(total_organisms)
        / (math.factorial(2) * math.factorial(total_organisms - 2))
        * 4
    )
    dominant = 0
    ...
    dominant += (
        math.factorial(homozygous_dominant)
        / (math.factorial(2) * math.factorial(homozygous_dominant - 2))
        * 4
    )
    ...
    dominant += (
        math.factorial(heterozygous)
        / (math.factorial(2) * math.factorial(heterozygous - 2))
        * 3
    )
...
def find_most_likely_common_ancestor(path_to_fasta: str) -> Tuple[str, dict]:
    """
    In “Counting Point Mutations”, we calculated the minimum number of symbol
    mismatches between two strings of equal length to model the problem of
    finding the minimum number of point mutations occurring on the evolutionary
    path between two homologous strands of DNA.
    If we instead have several homologous strands that we wish to analyze simultaneously,
    then the natural problem is to find an average-case strand to
    ...
    """
    fasta = fasta_to_dict(path_to_fasta)
    length = len(list(fasta.values())[0])
    bases = ("A", "C", "T", "G")
    consensus = [0 for _ in range(length)]
    profile = {x: [0 for _ in range(length)] for x in bases}

    for sequence in fasta.values():
        for index in range(length):
            profile[sequence[index]][index] += 1

    for index in range(length):
        max_occurance = 0
        for base in bases:
            if profile[base][index] > max_occurance:
                max_occurance = profile[base][index]
                consensus[index] = base
